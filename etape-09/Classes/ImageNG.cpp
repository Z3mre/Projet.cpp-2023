...
void ImageNG::Save(ofstream & fichier) const
{
    ...
    for (int i = 0; i < dimension.getLargeur(); ++i)
    {
        fichier.write((char *) matrice[i], dimension.getHauteur() * sizeof(int));
    }
}
...
void ImageNG::Load(ifstream & fichier)
{
    ...
    for (int i = 0; i < dimension.getLargeur(); ++i)
    {
        fichier.read((char *) matrice[i], dimension.getHauteur() * sizeof(int));
    }
}
 ...
 ostream& operator<<(ostream& s,const ImageNG& m)
{
	...
	s << "[id : "<< m.id << " | nom : " << m.getNom() << " | largeur : " << m.dimension.getLargeur() << " | hauteur : " << m.dimension.getHauteur() 
